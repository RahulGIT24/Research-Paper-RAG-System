...
function QueryPageContent() {
    ...
    const giveSources = (content: string) => {
        if (content == '') return []
        const sourcesStr = content.split("<SOURCES>")[1].trim().replace("</SOURCES>", "")
    ...
    const getChatMessages = async () => {
        ...
        try {
            ...
            const formattedMessages: Message[] = res.data.messages.map(
                (msg: { id: string, role: string, content: string }) => ({

                    id: msg.id.toString(),
                    role: msg.role,
                    content: msg.role === "assistant" ? msg.content.split("<SOURCES>")[0] : msg.content,
                    sources:
                        msg.role === "assistant"
                            ? giveSources(msg.content)
                            : [],
    ...
    useEffect(() => {
        (async () => await getChatMessages())();
    }, [searchParams])

    return (
        <div className="flex h-[calc(100vh-140px)] gap-0">
            <Sidebar setError={setError} setMessages={setMessages} sidebarOpen={sidebarOpen} />
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-3 py-2 rounded-xl text-slate-500 bg-slate-200 shadow-[4px_4px_8px_#c1c9d2,-4px_-4px_8px_#ffffff] hover:shadow-[inset_2px_2px_4px_#c1c9d2,inset_-2px_-2px_4px_#ffffff] transition-all duration-300 text-sm"
                        >
                            ☰
                        </button>
